...
> **Status: unvalidated draft (v0.1).** Everything below describes how to apply the rubric as written and how to read its output honestly. It is not a claim that the weights are correct — that's what the [validation plan](rubric.md#6-validation-plan) is for. Nothing in this guide should be read as staffing or budget advice.
...
**When to score.** At feasibility review, before the study is institutionally committed to — the score is meant to be an input to that go/no-go, not a justification written after the fact. Then annually, and again on any amendment that touches Domains 3 (visits/procedures), 4 (IP/intervention), 5 (data/specimens), or 6 (safety reporting). An amendment that only changes, say, the community advisory board arrangement (Domain 7) doesn't require a re-score under the rubric as written, though nothing stops you from doing one.
...
That's one domain, two items at their anchors and one interpolated, to show the method — not a template to reproduce across all 32 items. The rubric deliberately left the other anchors open for judgment; manufacturing a worked answer for each one here would just be inventing the calibration the validation plan hasn't done yet.

**Domain 5's cap.** Score all six items normally. The tool automatically caps the domain's contribution to the 100-point total at 16, even though the items themselves sum to a possible 18. You'll see both numbers if your raw sum exceeds the cap — that's expected ([why, in rubric.md](rubric.md#domain-5--data--specimen-management-016)).
...
**My Part A total doesn't match what I added up by hand.** Check Domain 5 — its contribution is capped at 16 even though the six items can sum to 18. The tool shows "raw → capped" whenever that's happening.
