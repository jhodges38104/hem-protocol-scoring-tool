...
Source: internal HEM CTM draft, [*Non-Malignant Hematology Protocol Complexity & Workload Rubric (Draft v0.1)*](TrialComplexityRubric.pdf) — the original PDF is included alongside this file. This write-up reorganizes and lightly expands that draft for readability and adds the [References](#references) section. It changes nothing the source PDF defines — every v0.1 weight, range, and formula is reproduced as written — but it is no longer limited to the PDF: v0.2 adds Domain 8 and the Part B data volume factor, neither of which appears in that document. See the change note below.
